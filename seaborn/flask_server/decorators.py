...
from ..flask_server.models.models import ApiModel
...
def api_endpoint(auth='Anonymous', validator=None, html=None, redirect=None, add=False, commit=False, delete=False,
    ...
    def endpoint_decorator(func):
        ...
        def decorated_function(*args, **kwargs):
            try:
                #1log.trace("Api Call to %s <%s>" % (path.split(RELATIVE_PATH, 1)[-1], func_name))
                ...
                #1log.trace("Api Call kwargs: %s" % str(kwargs))
                try:
                    try:
                        ret = func(**kwargs)
