...
#1 log.trace("Importing endpoint account.views")
...
def get_array(primary= False, limit=None, offset=None):
    """
    :param primary: bool if True will only return accounts the user is primary
    :param offset: int of the offset to use
    :param limit:  int of max number of puzzles to return
    :return:       list of Account dict the current user has access to
    """
    ...
    if primary:
        ...
        query = query.join(Access).filter(
...
def create(name=None, user_ids=None):
    """
    :param name:     str of name for the account,
        defaults to the created timestamp
    :param user_ids: list of int of users to give access to this account
    ...
    if account is None:
        ...
        if account.user_id is not current_user.user_id:
            raise UnauthorizedException(
                'Account with the name "%s" already exists'%name)
