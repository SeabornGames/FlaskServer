...
class Transfer(db.Model, ApiModel):
    ...
    deposit_account_id = db.Column(db.Integer,
                                   db.ForeignKey('account.account_id'))
    withdraw_account_id = db.Column(db.Integer,
                                    db.ForeignKey('account.account_id'))
    ...
    deposit_account = db.relationship(
        Account, foreign_keys='Transfer.deposit_account_id',
        backref=backref('account_deposits', lazy='dynamic', uselist=True))

    withdraw_account = db.relationship(
        Account, foreign_keys='Transfer.withdraw_account_id',
        backref=backref('account_withdraws', lazy='dynamic', uselist=True))

    @classmethod
    def keys(cls):
        return ['transfer_id', 'user_id', 'deposit_account_id',
                'deposit_account_name', 'withdraw_account_id',
                'withdraw_account_name', 'amount', 'created_timestamp',
                'receipt']
    ...
    def check_receipt(self, receipt):
        # todo implement some receipt validation
        # so users can't arbitrarily increase there account holdings
        return True

    def generate_receipt(self):
        self.receipt = "unclaimed"
        # todo implement some receipt validation scheme
    ...
    def validate_amount(self, amount, **kwargs):
        ...
        assert amount <= 1000000, \
            "Transfer amounts have to be less than a million dollars"
